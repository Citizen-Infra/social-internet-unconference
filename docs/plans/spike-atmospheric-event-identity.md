...
| # | Question |
|---|---|
| D1-Q1 | **Resolved and provisioned:** use the organizer-controlled [`unconference.bsky.social`](https://bsky.app/profile/unconference.bsky.social) account, DID `did:plc:mzvqnxye3oejamuwmfl4qvou`, as the event identity. The DID/PDS can migrate independently of Community Admin; organizers retain account recovery and custody. |
...
| D3-Q1 | **Resolved:** use unauthenticated `app.bsky.graph.getRelationships` with actor = SIU DID and participant DIDs in `others`, matching the shipped AV1 producer. `followedBy` proves each participant follows SIU; `blocking` and `blockedBy` provide both-direction block state. The endpoint accepts up to 30 target DIDs, so it supports point checks without follower-list pagination. Respect AppView rate-limit headers and 429 backoff; cache a positive or negative decision for a bounded TTL. |
...
| D3-Q3 | **Resolved for AV2:** Community Admin's existing event-grant store is authoritative during the bridge. Avails receives an online, capability-specific decision rather than a signed or replayable credential. A portable signed representation remains AV3 work and must not call the relationship community membership. |
| D3-Q4 | **Resolved for AV2:** support and Avails introspection call one Community Admin decision function with one five-minute source TTL. Avails uses a `ca-event` scope keyed by the SIU DID and never queries the Bluesky relationship graph. Exact contract: [SIU AV2 Event-Grant Consumer Contract](./siu-av2-event-grant-consumer-contract.md). |
